---
title: "SPRO Takeaway Cups"
subtitle: "Take the vibes with you"
image: 'img/spro/cups-thumb.jpg'
description: "3 day freestyle doodle mural at SPRO Coffee Co. in Brighton and Hove. It’s my biggest mural to date. It took 3 days and was a ton of fun."
categories: ['Illustration']
tags: ['Client Work', 'Illustration', 'IRL']
background_color: "#4878a0"
color: "white"
date: 2024-11-29
related:
  - /art/spro-mural
  - /art/phone-cover-happy-yellow
  - /art/phone-cover-are-you-ugly
grid_images_1:
  - "img/spro/8oz-stopmo--540x675.gif?v=2"
  - "img/spro/6oz-stopmo--540x675.gif?v=1"
  - "img/spro/4oz-stopmo--540x675.gif?v=1"
grid_images_2:
  - "img/spro/cup-still-4.jpg?v=1"
  - "img/spro/cup-still-3.jpg?v=1"
  - "img/spro/cup-still-1.jpg?v=1"
grid_images_3:
  - "img/spro/cup-still-2.jpg?v=1"
  - "img/spro/cup-still-5.jpg?v=1"
  - "img/spro/cup-still-6.jpg?v=2"
---

{% include img.liquid img="img/spro/cups-banner.jpg" caption="Papa bear, mama bear and baby bear SPRO cups" %}

The takeaway cups needed a distinct vibe—and to match [the muraled walls.](/spro-mural/)

With this style, mural painting and vector drawing are really similar.  
These cups, along with a bunch of other collateral, are the bedrock of the SPRO vibe.

Check out the reel [on Instagram.](https://www.instagram.com/p/DUs1c2cjTlz/)

{% include grid-images.html imgs=page.grid_images_1 %}
{% include img.liquid img="img/spro/cup-landscape-3.jpg?v=1" %}
{% include grid-images.html imgs=page.grid_images_2 %}
{% include grid-images.html imgs=page.grid_images_3 %}
{% include img.liquid img="img/spro/cup-landscape-1.jpg?v=1" %}
{% include img.liquid img="img/spro/cup-landscape-2.jpg?v=1" %}

### Find the cups
- On [Bēhance.](https://www.behance.net/gallery/244085415/SPRO-Takeaway-Cups)
- On [Instagram.](https://www.instagram.com/p/DUs1c2cjTlz/)
- [IRL in Brighton & Hove.](https://maps.app.goo.gl/KnA7WeyhRmcquzkN8)